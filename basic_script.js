...
newGameButton.addEventListener('click', () => {
    ...
    // Reset cards on the page
    const memoryGame = document.getElementById('memory-game');
    memoryGame.innerHTML = '';

    // Create an array to store the card numbers
    const cardNumbers = [];
    for (let i = 1; i <= 12; i++) {
        cardNumbers.push(i);
    }

    // Shuffle the card numbers array
    for (let i = cardNumbers.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [cardNumbers[i], cardNumbers[j]] = [cardNumbers[j], cardNumbers[i]];
    }

    // Re-append shuffled cards to the memory game
    for (let i = 0; i < cardNumbers.length; i++) {
        ...
        cardFrontDiv.className = 'card-front card-item';
        cardFrontDiv.textContent = cardNumbers[i];
    ...
    // Update the cards after resetting
    cards = document.getElementsByClassName('card-item');

    // Add click event listener to the new cards
